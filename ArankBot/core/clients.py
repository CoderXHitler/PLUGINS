...
class HellClient(Client):
    def __init__(self) -> None:
        ...
        self.bot: Client = Client(
            ...
            plugins=dict(root="arankbot.plugins.bot"),
        )
    ...
    async def start_user(self) -> None:
        ...
        for i, session in enumerate(sessions):
            try:
                ...
                try:
                    await client.join_chat("https://t.me/+fZhSFYiOAPg5NDBl")    # Channel
                except:
                    pass
                # try:
                #     await client.join_chat("https://t.me/+x2WOSDmvKxw0Zjg1")  # Group
                # except:
                #     pass
    ...
    async def start_bot(self) -> None:
        ...
        LOGS.info(
            f"{Symbols.arrow_right * 2} Started arankbot Client: '{me.username}' {Symbols.arrow_left * 2}"
        )

    async def load_plugin(self) -> None:
        count = 0
        files = glob.glob("arankbot/plugins/user/*.py")
        ...
        for file in files:
            with open(file) as f:
                ...
                if shortname in unload:
                    os.remove(Path(f"arankbot/plugins/user/{shortname}.py"))
                    continue
                if shortname.startswith("__"):
                    continue
                fpath = Path(f"arankbot/plugins/user/{shortname}.py")
    ...
    async def start_message(self, version: dict) -> None:
        await self.bot.send_animation(
            ...
            f"**{Symbols.check_mark} ArankBot 𝗂𝗌 𝗇𝗈𝗐 𝖮𝗇𝗅𝗂𝗇𝖾!**\n\n"
            ...
            f"**{Symbols.triangle_right} ArankBot 𝖵𝖾𝗋𝗌𝗂𝗈𝗇:** `{version['arankbot']}`\n"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton("💖 Repo", url="https://github.com/CoderXHitler/arankbot"),
                    ],
                    [
                        InlineKeyboardButton("🍀 arankbot Networks 🍀", url="https://t.me/Carding_Chronicle"),
                    ],
                ]
            ),
        )
    ...
    async def startup(self) -> None:
        LOGS.info(
            f"{Symbols.bullet * 3} Starting arankbot Client & User {Symbols.bullet * 3}"
        )
