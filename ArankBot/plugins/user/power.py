...
from Arankbot.functions.tools import restart, gen_changelogs, initialize_git
...
async def restart_bot(_, message: Message):
    await Arankbot.edit(message, "💫 Restarted Bot Successfully!")
    try:
        if HEROKU_APP:
            try:
...
async def shutdown_bot(_, message: Message):
    await Arankbot.edit(
        message,
        "**[ ⚠️ ]** __ArankBot is now offline! Manually start again to get it back online.__",
    )
    try:
        if HEROKU_APP:
            try:
...
async def clenup_bot(_, message: Message):
    await Arankbot.edit(message, "**♻️ ArankBot Cleanup Completed!**")
    await restart(clean_up=True)


@on_message("update", allow_stan=True)
async def update_bot(_, message: Message):
    Arank = await Arankbot.edit(message, "**🔄 𝖨𝗇 𝖯𝗋𝗈𝗀𝗋𝖾𝗌𝗌...**")

    if len(message.command) < 2:
        status, repo, force = await initialize_git(Config.PLUGINS_REPO)
        if not status:
            return await Arankbot.error(Arank, repo)
        ...
        if not changelogs and not force:
            ...
            return await Arankbot.delete(
                Arank, "__There are no updates available right now.__"
            )
    ...
    if cmd == "plugins":
        ...
        if HEROKU_APP:
            ...
            try:
                ...
                repo.__del__()
                return await Arankbot.error(Arank, f"__Invalid Heroku Creds:__ `{e}`")
        ...
        return await Arankbot.delete(
            Arank, f"**[ ⚠️ ]** __Invalid update argument:__ `{cmd}`"
        )
