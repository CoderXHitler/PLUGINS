...
async def lockGC(client: Client, message: Message):
    if not message.chat.type == ChatType.SUPERGROUP:
        return await Arankbot.delete(message, "Only supergroups are supported.")

    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me something to lock.")
    ...
    if lock_type == "all":
        ...
        return await Arankbot.delete(message, "Invalid lock type.")
...
async def unlockGC(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me something to unlock.")
    ...
    await Arankbot.edit(message, f"**Unlocked {lock_type} for {message.chat.title}**")
