...
async def instagramReels(_, message: Message):
    ...
    if not isInstagramLink(query):
        return await Arankbot.error(Arank, "Give a valid instagram reels link.")

    try:
        driver, _ = Driver.get()
        if not driver:
            return await Arankbot.error(Arank, _)
        ...
        if reels:
            await Arank.edit("Found the reel. **Downloading...**")
...
async def instagramPost(_, message: Message):
    ...
    if not isInstagramLink(query):
        return await Arankbot.error(Arank, "Give a valid instagram post link.")
...
async def instagramUser(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give an instagram username to fetch info.")

    BASE_URL = "https://i.instagram.com/api/v1/users/web_profile_info/"

    query = (await Arankbot.input(message)).replace("@", "").strip()
    Arank = await Arankbot.edit(message, f"**Searching** `{query}` **on instagram**...")
