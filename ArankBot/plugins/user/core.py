...
from Arankbot.core import ENV, Config, Symbols

from . import HelpMenu, bot, db, handler, Arankbot, on_message
...
async def help(client: Client, message: Message):
    ...
    if len(message.command) == 1:
        try:
            ...
            return await Arank.delete()
        except Exception as e:
            await Arankbot.error(Arank, str(e), 20)
            return

    plugin = await Arankbot.input(message)
    if plugin.lower() in Config.CMD_MENU:
        try:
            await Arankbot.edit(
                Arank, Config.CMD_MENU[plugin.lower()], ParseMode.MARKDOWN
            )
            return
        except Exception as e:
            await Arankbot.error(Arank, str(e), 20)
    ...
    await Arankbot.edit(Arank, available_plugins, ParseMode.MARKDOWN)


@on_message("repo", allow_stan=True)
async def repo(_, message: Message):
    REPO_TEXT = (
        "__🍀 𝖱𝖾𝗉𝗈:__ [Github](https://github.com/CoderXHitler/ArankBot)\n\n"
        "__🍀 Updates:__ @Its_ArankBot\n"
        "__🍀 Support:__ @ArankBot_Chats\n\n"
        "**By ©️ @ArankBot_Networks**"
    )
    await Arankbot.edit(message, REPO_TEXT, no_link_preview=True)
...
async def plugin_info(_, message: Message):
    ...
    if plugin.lower() in Config.CMD_MENU:
        try:
            ...
            await Arankbot.error(message, str(e), 20)
            return
    await Arankbot.error(message, f"**Invalid Plugin Name:** `{plugin}`", 20)
...
async def command_info(_, message: Message):
    ...
    if cmd.lower() in Config.CMD_INFO:
        try:
            ...
            await Arankbot.edit(message, template, ParseMode.MARKDOWN)
            return
        except Exception as e:
            await Arankbot.error(message, str(e), 20)
            return
    await Arankbot.error(message, f"**Invalid Command Name:** `{cmd}`", 20)


@on_message("send", allow_stan=True)
async def send_plugin(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me a plugin name to send.")

    plugin = message.command[1].lower().replace(".py", "").strip()
    if plugin not in Config.CMD_MENU:
        return await Arankbot.delete(message, f"**Invalid Plugin Name:** `{plugin}`")

    try:
        await client.send_document(
            message.chat.id,
            f"./Arankbot/plugins/user/{plugin}.py",
            caption=f"**🍀 𝖯𝗅𝗎𝗀𝗂𝗇:** `{plugin}.py`",
        )
        await Arankbot.delete(message, f"**Sent:** `{plugin}.py`")
    except Exception as e:
        await Arankbot.error(message, str(e), 20)
...
async def install_plugins(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.document:
        return await Arankbot.delete(message, "Reply to a plugin to install it.")

    Arank = await Arankbot.edit(message, "**Installing...**")
    plugin_path = await message.reply_to_message.download("./Arankbot/plugins/user/")

    if not plugin_path.endswith(".py"):
        os.remove(plugin_path)
        return await Arankbot.error(Arank, "**Invalid Plugin:** Not a python file.", 20)

    plugin = plugin_path.split("/")[-1].replace(".py", "").strip()
    if plugin in Config.CMD_MENU:
        os.remove(plugin_path)
        return await Arankbot.error(
            Arank, f"**Plugin Already Installed:** `{plugin}.py`", 20
        )
    ...
    try:
        ...
        path = Path(f"Arankbot/plugins/user/{shortname}.py")
        name = "Arankbot.plugins.user." + shortname
        spec = importlib.util.spec_from_file_location(name, path)
        load = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(load)
        sys.modules["Arankbot.plugins.user." + shortname] = load
        await Arankbot.edit(Arank, f"**Installed:** `{plugin}.py`")
    except Exception as e:
        await Arankbot.error(Arank, str(e), 20)
        os.remove(plugin_path)
...
async def uninstall_plugins(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me a plugin name to uninstall.")
...
async def installall(client: Client, message: Message):
    ...
    try:
        ...
        return await Arankbot.delete(message, f"**Invalid Channel Username:** `{e}`")

    Arank = await Arankbot.edit(message, f"**Installing plugins from {chat.title}...**")
    ...
    async for msg in client.search_messages(chat.id, filter=MessagesFilter.DOCUMENT):
        if msg.document.file_name.endswith(".py"):
            dwl_path = await msg.download("./Arankbot/plugins/user/")
    ...
    await Arank.edit(finalStr, ParseMode.MARKDOWN, disable_web_page_preview=True)


@on_message("unload", allow_stan=True)
async def unload_plugins(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me a plugin name to unload.")
    ...
    if plugin in unloaded:
        return await Arankbot.delete(message, f"**Already Unloaded:** `{plugin}.py`")

    unloaded.append(plugin)
    await db.set_env(ENV.unload_plugins, " ".join(unloaded))
    await Arankbot.delete(
...
async def load_plugins(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me a plugin name to load.")

    plugin = message.command[1].lower().replace(".py", "").strip()
    unloaded = await db.get_env(ENV.unload_plugins) or ""
    unloaded = unloaded.split(" ")
    if plugin not in unloaded:
        return await Arankbot.delete(message, f"**Already Loaded:** `{plugin}.py`")
...
HelpMenu("help").add(
    ...
    "installall @plugins_for_Arankbot",
    "Do not install plugins from untrusted sources, they can be a malware. We're not responsible for any damage caused by them.",
).add(
    "unload",
    "<plugin name>",
