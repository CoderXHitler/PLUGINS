...
from Arankbot.functions.driver import Climate
from Arankbot.functions.formatter import subscript, superscript
from Arankbot.functions.templates import airpollution_templates, climate_templates
...
async def climate(_, message: Message):
    if len(message.command) < 2:
        ...
        city = await Arankbot.input(message)

    Arank = await Arankbot.edit(message, f"**Fetching climate data for `{city}`**")

    apiKey = await db.get_env(ENV.climate_api)
    if not apiKey:
        return await Arankbot.delete(
            Arank, f"**Climate API not found!** Setup `{ENV.climate_api}` first."
        )

    data = await Climate.fetchWeather(city, apiKey)
    if not data:
        return await Arankbot.delete(Arank, f"**Climate data not found for `{city}`**")
...
async def airpollution(_, message: Message):
    ...
    Arank = await Arankbot.edit(message, f"**Fetching air pollution data for `{city}`**")
    ...
    if not data:
        return await Arankbot.delete(
            Arank, f"**Air pollution data not found for `{city}`**"
        )
