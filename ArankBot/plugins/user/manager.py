...
from Arankbot import HEROKU_APP
from Arankbot.core.config import all_env, os_configs
from Arankbot.functions.paste import spaceBin
from Arankbot.functions.templates import usage_templates
from Arankbot.functions.tools import restart, update_dotenv
...
async def getvar(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give a varname to fetch value.")
    ...
    if isinstance(value, str):
        ...
        await Arankbot.delete(message, f"**𝖵𝖺𝗋𝗂𝖺𝖻𝗅𝖾 {varname} 𝖽𝗈𝖾𝗌 𝗇𝗈𝗍 𝖾𝗑𝗂𝗌𝗍𝗌!**")
...
async def getallvar(_, message: Message):
    ...
    await Arankbot.edit(message, text)
...
async def delvar(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "**𝖦𝗂𝗏𝖾 𝗏𝖺𝗋𝗇𝖺𝗆𝖾 𝖺𝗅𝗈𝗇𝗀 𝗐𝗂𝗍𝗁 𝗍𝗁𝖾 𝖼𝗈𝗆𝗆𝖺𝗇𝖽!**")
    ...
    await Arankbot.delete(message, "**𝖭𝗈 𝗌𝗎𝖼𝗁 𝗏𝖺𝗋𝗂𝖺𝖻𝗅𝖾 𝖿𝗈𝗎𝗇𝖽 𝗂𝗇 𝖽𝖺𝗍𝖺𝖻𝖺𝗌𝖾 𝗍𝗈 𝖽𝖾𝗅𝖾𝗍𝖾!**")


@on_message("usage", allow_stan=True)
async def dynoUsage(_, message: Message):
    Arank = await Arankbot.edit(message, "Fetching usage info...")
    ...
    if HEROKU_APP:
        ...
        if response.status_code != 200:
            return await Arankbot.error(message, "Something went wrong!")
...
async def getLogs(_, message: Message):
    ...
    try:
        if os.path.exists("ArankBot.log"):
            with open("ArankBot.log", "r") as file:
        ...
        await Arankbot.error(message, str(e))
