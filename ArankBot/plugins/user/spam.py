...
async def spamMessage(client: Client, message: Message):
    if len(message.command) < 3:
        return await Arankbot.delete(message, "Give me something to spam.")

    reply_to = message.reply_to_message.id if message.reply_to_message else None
    try:
        count = int(message.command[1])
    except ValueError:
        return await Arankbot.delete(message, "Give me a valid number to spam.")
...
async def delaySpam(client: Client, message: Message):
    ...
    try:
        ...
        return await Arankbot.delete(message, "Give me a valid delay to spam.")
...
async def mediaSpam(client: Client, message: Message):
    if not message.reply_to_message:
        return await Arankbot.delete(message, "Reply to a media to spam.")
...
async def stopSpam(_, message: Message):
    ...
    if not spamTask.get(chat_id, None):
        return await Arankbot.delete(message, "No spam task running for this chat.")
    ...
    await Arankbot.delete(message, f"Spam task stopped for {chat_name}.")
