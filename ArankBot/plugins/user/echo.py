...
async def echo(client: Client, message: Message):
    ...
    if await db.is_echo(client.me.id, message.chat.id, user):
        return await Arankbot.delete(message, "Echo is already enabled for this user!")

    await db.set_echo(client.me.id, message.chat.id, user)
    await Arankbot.delete(message, "Echo enabled for this user!")
...
async def unecho(client: Client, message: Message):
    ...
    if not await db.is_echo(client.me.id, message.chat.id, user):
        return await Arankbot.delete(message, "Echo is already disabled for this user!")

    await db.rm_echo(client.me.id, message.chat.id, user)
    await Arankbot.delete(message, "Echo disabled for this user!")
...
async def listecho(client: Client, message: Message):
    ...
    if not echos:
        return await Arankbot.delete(message, "No echos in this chat!")
