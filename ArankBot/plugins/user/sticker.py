...
from Arankbot.functions.convert import image_to_sticker, video_to_sticker
from Arankbot.functions.sticker import (
    add_sticker,
    check_sticker_data,
    create_sticker,
    get_emoji_and_id,
...
from . import Config, HelpMenu, Symbols, db, Arankbot, on_message


@on_message("kang", allow_stan=True)
async def kangSticker(client: Client, message: Message):
    if not message.reply_to_message:
        return await Arankbot.delete(message, "Reply to a sticker to kang it.")

    Arank = await Arankbot.edit(message, "__Kanging sticker...__")
    ...
    if pack_type is None:
        return await Arankbot.delete(Arank, "Unsupported media type.")
    ...
    pack_url_suffix = (
        f"HB{client.me.id}_vol{pack_id}_{pack_type}_by_{Arankbot.bot.me.username}"
    )
    ...
    if message.reply_to_message.sticker:
        if is_static:
            ...
            if not status:
                return await Arankbot.error(Arank, path)
        else:
            path = await message.reply_to_message.download(Config.TEMP_DIR)
    else:
        if is_video:
            await Arank.edit("Converting to video sticker...")
            path, status = await video_to_sticker(message.reply_to_message)
            if not status:
                return await Arankbot.error(Arank, path)
        elif is_animated:
            await Arank.edit("Converting to animated sticker...")
            path = await message.reply_to_message.download(Config.TEMP_DIR)
        else:
            await Arank.edit("Converting to sticker...")
            file = await message.reply_to_message.download(Config.TEMP_DIR)
            status, path = await image_to_sticker(file)
            if not status:
                return await Arankbot.error(Arank, path)

    sticker = await create_sticker(Arankbot.bot, Config.LOGGER_ID, path, pack_emoji)
    os.remove(path)

    try:
        while True:
            stickerset = await get_sticker_set(Arankbot.bot, pack_url_suffix)
            if stickerset:
                if stickerset.set.count == pack_limit:
                    ...
                    pack_url_suffix = f"HB{client.me.id}_vol{pack_id}_{pack_type}_by_{Arankbot.bot.me.username}"
                    continue
                else:
                    await add_sticker(Arankbot.bot, stickerset, sticker)
            else:
        ...
        return await Arankbot.delete(
            Arank, f"Start @{Arankbot.bot.me.username} first and try to kang again.", 20
        )
...
async def packKang(client: Client, message: Message):
    if not message.reply_to_message:
        return await Arankbot.delete(message, "Reply to a sticker to kang whole pack!")

    Arank = await Arankbot.edit(message, "__Kanging sticker pack...__")

    pack_id = 1
    nickname = f"@{client.me.username}" if client.me.username else client.me.first_name
    packname = await Arankbot.input(message) or f"{nickname}'s Pack (Vol.{pack_id})"
    pack_url_suffix = f"HB{client.me.id}_pkvol{pack_id}_by_{Arankbot.bot.me.username}"

    if not message.reply_to_message.sticker:
        return await Arankbot.delete(Arank, "Reply to a sticker to kang whole pack!")
    ...
    replied_set = await get_sticker_set(Arankbot.bot, replied_setname)
    if not replied_set:
        return await Arankbot.delete(Arank, "Reply to a sticker to kang whole pack!")
    ...
    try:
        while True:
            ...
            if stickerset:
                pack_id += 1
                pack_url_suffix = (
                    f"HB{client.me.id}_pkvol{pack_id}_by_{Arankbot.bot.me.username}"
                )
                packname = (
                    await Arankbot.input(message) or f"{nickname}'s Pack (Vol.{pack_id})"
                )
                continue
            else:
                await new_sticker_set(
...
async def stickerInfo(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.sticker:
        return await Arankbot.delete(message, "Reply to a sticker to get their info.")

    Arank = await Arankbot.edit(message, "__Fetching sticker info ...__")

    sticker = message.reply_to_message.sticker

    sticker_set = await get_sticker_set(Arankbot.bot, sticker.set_name)
    if not sticker_set:
        return await Arankbot.delete(Arank, "This sticker is not part of a pack.")
...
async def removeSticker(_, message: Message):
    ...
    Arank = await Arankbot.edit(message, "__Removing sticker from pack...__")
    ...
    try:
        await remove_sticker(Arankbot.bot, sticker.file_id)
        await Arankbot.delete(
            Arank, f"**𝖣𝖾𝗅𝖾𝗍𝖾𝖽 𝗍𝗁𝖾 𝗌𝗍𝗂𝖼𝗄𝖾𝗋 𝖿𝗋𝗈𝗆 𝗉𝖺𝖼𝗄:** {sticker_set.set.title}",
        )
    except Exception as e:
