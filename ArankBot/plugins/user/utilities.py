...
from Arankbot.functions.images import remove_bg
from Arankbot.functions.media import get_media_text_ocr
from Arankbot.functions.paste import spaceBin
...
async def readImage(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await Arankbot.delete(message, "Reply to a photo to read text on it.")
...
async def removeBg(_, message: Message):
    ...
    if message.reply_to_message:
        ...
        resp = requests.get(await Arankbot.input(message))
    ...
    Arank = await Arankbot.edit(message, "Removing background...")
...
async def paste_text(_, message: Message):
    Arank = await Arankbot.edit(message, "Pasting text...")
    text_to_paste = ""
    extention = "none"

    if len(message.command) >= 2:
        text_to_paste = await Arankbot.input(message)
        ...
        return await Arankbot.delete(message, "Reply to a text to paste it.")
...
async def currencyAPI(_, message: Message):
    if len(message.command) < 3:
        return await Arankbot.delete(message, "Give currency code to get it's value.")
    ...
    if not apikey:
        return await Arankbot.delete(message, "Please setup currency api key.")

    Arank = await Arankbot.edit(message, "Getting currency value...")
    ...
    if data["result"] == "success":
        ...
        await Arankbot.error(Arank, f"**Error:** `{data['error-type']}`")
...
async def currencyAPI2(_, message: Message):
    ...
    try:
        ...
        return await Arankbot.delete(message, "Give amount in numbers.")
...
async def currencyCodes(_, message: Message):
    Arank = await Arankbot.edit(message, "Getting currency codes...")

    apikey = await db.get_env(ENV.currency_api)
    if not apikey:
        return await Arankbot.delete(Arank, "Please setup currency api key.")
