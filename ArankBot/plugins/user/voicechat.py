...
from . import HelpMenu, Symbols, group_only, Arankbot, on_message
...
async def startvc(client: Client, message: Message):
    if len(message.command) > 1:
        call_name = await Arankbot.input(message)
    else:
        call_name = "Arankbot VC"

    Arank = await Arankbot.edit(message, "Starting Voice Chat...")
    try:
        await client.invoke(
            CreateGroupCall(
        ...
        await Arankbot.delete(Arank, "Voice Chat started!")
...
async def endvc(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "Ending Voice Chat...")
    ...
    try:
        ...
        await Arankbot.delete(Arank, "Voice Chat ended!")
    except Exception as e:
        await Arankbot.error(Arank, str(e))
...
async def vclink(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "Getting Voice Chat link...")
    ...
    try:
        ...
        await Arankbot.delete(Arank, f"Voice Chat Link: {invite.link}")
    except Exception as e:
...
async def vcmembers(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "Getting Voice Chat members...")
