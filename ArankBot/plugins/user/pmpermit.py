...
async def block_user(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        return await Arankbot.delete(message, "`I can't block myself`")

    if user.id in Config.DEVS:
        return await Arankbot.delete(message, "`I can't block my devs`")
    ...
    if success:
        ...
        await Arankbot.error(message, f"`Couldn't block {user.mention}`")
...
async def unblock_user(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        return await Arankbot.delete(message, "`I can't unblock myself`")
    ...
    if success:
        ...
        await Arankbot.error(message, f"`Couldn't unblock {user.mention}`")
...
async def allow_pm(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Arankbot.delete(message, "`I can't allow myself`")

    if await db.is_pmpermit(client.me.id, user_id):
        return await Arankbot.delete(message, "`User is already allowed to pm!`")

    await db.add_pmpermit(client.me.id, user_id)
    await Arankbot.delete(message, f"**{Symbols.check_mark} Allowed:** {user_mention}")
...
async def disallow_pm(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Arankbot.delete(message, "`I can't disallow myself`")

    if not await db.is_pmpermit(client.me.id, user_id):
        return await Arankbot.delete(message, "`User is not allowed to pm!`")
...
async def allowlist(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "`Fetching allowlist...`")
    users = await db.get_all_pmpermits(client.me.id)
    if not users:
        return await Arankbot.delete(Arank, "`No users allowed to pm!`")
...
async def set_pmpermit(_, message: Message):
    if len(message.command) < 2:
        ...
        return await Arankbot.delete(message, f"**Current PM Permit Setting:** `{text}`\n\nTo change the setting give either `on` or `off` as argument.")

    cmd = message.command[1].lower().strip()

    if cmd == "on":
        await db.set_env(ENV.pmpermit, True)
        await Arankbot.delete(message, "**PM Permit Enabled!**")
    elif cmd == "off":
        await db.set_env(ENV.pmpermit, False)
        await Arankbot.delete(message, "**PM Permit Disabled!**")
    else:
        await Arankbot.delete(message, "**Invalid Argument!**")
