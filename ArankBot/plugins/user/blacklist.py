...
from Arankbot.core import Config, Symbols
from Arankbot.functions.utility import BList

from . import HelpMenu, custom_handler, db, Arankbot, on_message


@on_message("blacklist", admin_only=True, allow_stan=True)
async def blacklist(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me something to blacklist.")

    text = await Arankbot.input(message)

    if await db.is_blacklist(client.me.id, message.chat.id, text):
        return await Arankbot.delete(message, f"**Already blacklisted** `{text}`")

    await BList.addBlacklist(client.me.id, message.chat.id, text)
    await Arankbot.delete(message, f"**Blacklisted:** `{text}`")


@on_message("unblacklist", admin_only=True, allow_stan=True)
async def unblacklist(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give me something to unblacklist.")

    text = await Arankbot.input(message)

    if not await db.is_blacklist(client.me.id, message.chat.id, text):
        return await Arankbot.delete(message, f"`{text}` does not exist in blacklist.")

    await BList.rmBlacklist(client.me.id, message.chat.id, text)
    await Arankbot.delete(message, f"**Unblacklisted:** `{text}`")
...
async def blacklists(client: Client, message: Message):
    ...
    if not blacklists:
        return await Arankbot.delete(message, "No blacklists found.")
...
HelpMenu("blacklist").add(
    ...
    "unblacklist", "<text>", "Remove the text from blacklist.", "unblacklist Aranko"
).add(
