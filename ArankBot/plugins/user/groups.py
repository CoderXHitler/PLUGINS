...
from Arankbot.functions.media import get_media_fileid
from Arankbot.functions.templates import chat_info_templates

from . import HelpMenu, group_n_channel, Arankbot, on_message
...
async def setgpic(_, message: Message):
    ...
    if not status:
        return await Arankbot.delete(message, "𝖲𝗈𝗋𝗋𝗒, 𝗌𝗈𝗆𝖾𝗍𝗁𝗂𝗇𝗀 𝗐𝖾𝗇𝗍 𝗐𝗋𝗈𝗇𝗀.")

    await Arankbot.delete(message, "𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝗌𝖾𝗍 𝗀𝗋𝗈𝗎𝗉 𝗉𝗋𝗈𝖿𝗂𝗅𝖾 𝗉𝗂𝖼𝗍𝗎𝗋𝖾.")
    await Arankbot.check_and_log(
        "setgpic",
...
async def setgtitle(_, message: Message):
    ...
    new_title = await Arankbot.input(message)
    status = await message.chat.set_title(new_title)
    if not status:
        return await Arankbot.delete(message, "𝖲𝗈𝗋𝗋𝗒, 𝗌𝗈𝗆𝖾𝗍𝗁𝗂𝗇𝗀 𝗐𝖾𝗇𝗍 𝗐𝗋𝗈𝗇𝗀.")

    await Arankbot.delete(message, "𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝗌𝖾𝗍 𝗀𝗋𝗈𝗎𝗉 𝗍𝗂𝗍𝗅𝖾.")
    await Arankbot.check_and_log(
        "setgtitle",
        f"**Group Title**\n\n**Group:** `{prev_title}`\n**Group ID:** `{message.chat.id}`\n**Admin:** `{message.from_user.mention}`",
    )
...
async def setgabout(_, message: Message):
    ...
    new_about = await Arankbot.input(message)
    status = await message.chat.set_description(new_about)
    if not status:
        return await Arankbot.delete(message, "𝖲𝗈𝗋𝗋𝗒, 𝗌𝗈𝗆𝖾𝗍𝗁𝗂𝗇𝗀 𝗐𝖾𝗇𝗍 𝗐𝗋𝗈𝗇𝗀.")

    await Arankbot.delete(message, "𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝗌𝖾𝗍 𝗀𝗋𝗈𝗎𝗉 𝖺𝖻𝗈𝗎𝗍.")
    await Arankbot.check_and_log(
        "setgabout",
        f"**Group About**\n\n**Group:** `{message.chat.title}`\n**Group ID:** `{message.chat.id}`\n**Admin:** `{message.from_user.mention}`",
    )
...
async def setgusername(client: Client, message: Message):
    ...
    if user_status != ChatMemberStatus.OWNER:
        return await Arankbot.delete(message, "𝖨 𝖺𝗆 𝗇𝗈𝗍 𝗍𝗁𝖾 𝗈𝗐𝗇𝖾𝗋 𝗈𝖿 𝗍𝗁𝗂𝗌 𝗀𝗋𝗈𝗎𝗉.")
    ...
    new_username = await Arankbot.input(message)
    status = await client.set_chat_username(message.chat.id, new_username)
    if not status:
        return await Arankbot.delete(message, "𝖲𝗈𝗋𝗋𝗒, 𝗌𝗈𝗆𝖾𝗍𝗁𝗂𝗇𝗀 𝗐𝖾𝗇𝗍 𝗐𝗋𝗈𝗇𝗀.")

    await Arankbot.delete(message, "𝖲𝗎𝖼𝖼𝖾𝗌𝗌𝖿𝗎𝗅𝗅𝗒 𝗌𝖾𝗍 𝗀𝗋𝗈𝗎𝗉 𝗎𝗌𝖾𝗋𝗇𝖺𝗆𝖾.")
    await Arankbot.check_and_log(
        "setgusername",
        f"**Group Username**\n\n**Group:** `{message.chat.title}`\n**Group ID:** `{message.chat.id}`\n**Admin:** `{message.from_user.mention}`",
    )
...
async def getglink(_, message: Message):
    ...
    await Arankbot.delete(message, f"**𝖦𝗋𝗈𝗎𝗉 𝗅𝗂𝗇𝗄:** `{link}`")


@on_message("kickme", chat_type=group_n_channel, allow_stan=True)
async def kickme(client: Client, message: Message):
    Arank = await Arankbot.edit(message, random.choice(kickme_quotes))
    try:
        await client.leave_chat(message.chat.id)
    except Exception as e:
        return await Arankbot.delete(
            Arank, f"Deym! Can't leave this chat.\n**Error:** `{e}`"
        )
...
async def new_group(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "𝖨 𝗇𝖾𝖾𝖽 𝗌𝗈𝗆𝖾𝗍𝗁𝗂𝗇𝗀 𝗍𝗈 𝗌𝖾𝗍 𝖺𝗌 𝗀𝗋𝗈𝗎𝗉 𝗍𝗂𝗍𝗅𝖾.")

    new_title = await Arankbot.input(message)

    try:
        new_group = await client.create_group(new_title, Arankbot.bot.me.id)
        await Arankbot.edit(
            message, f"**𝖦𝗋𝗈𝗎𝗉 𝗅𝗂𝗇𝗄:** [{new_group.title}]({new_group.invite_link})"
        )
    except Exception as e:
        await Arankbot.error(message, f"`{e}`", 20)
...
async def new_channel(client: Client, message: Message):
    ...
    try:
        new_channel = await client.create_channel(new_title, "Created by ArankBot")
        await Arankbot.edit(
...
async def chatInfo(client: Client, message: Message):
    if len(message.command) > 1:
        try:
            ...
            return await Arankbot.error(message, f"`{e}`")
    else:
        chat = message.chat

    Arank = await Arankbot.edit(message, "Fetching chat info...")
    ...
    if chat.photo:
        ...
        await Arank.edit(chat_info, disable_web_page_preview=True)
...
async def chatAdmins(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Fetching chat admins...")
    ...
    await Arank.edit(admins, disable_web_page_preview=True)
...
async def chatBots(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Fetching chat bots...")
    ...
    await Arank.edit(bots, disable_web_page_preview=True)
...
async def chatId(_, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Fetching message info...")
    ...
    await Arank.edit(info, disable_web_page_preview=True)
...
async def inviteUser(client: Client, message: Message):
    ...
    users = (await Arankbot.input(message)).split(" ")
    Arank = await Arankbot.edit(message, "Inviting users...")

    resolved_users = await client.get_users(users)
    await message.chat.add_members([user.id for user in resolved_users])

    await Arank.edit("Successfully invited users to this chat.")
...
HelpMenu("groups").add(
    ...
    "newgroup", "<title>", "Create a new group.", "newgroup ArankBot Group"
).add(
    "newchannel", "<title>", "Create a new channel.", "newchannel ArankBot Channel"
).add(
    ...
    "chatadmins @Arankbot_Chats",
).add(
    "chatbots",
    "<chat id (optional)>",
    "Get the list of bots of mentioned chat.",
    "chatbots @Arankbot_Chats",
).add(
    "id",
