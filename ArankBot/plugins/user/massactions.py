...
from . import HelpMenu, group_n_channel, group_only, Arankbot, on_message
...
async def banall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            ...
            return await Arankbot.error(message, f"__Invalid chatId.__\n\n`{e}`")
    ...
    Arank = await Arankbot.edit(message, f"__Banning all users in {chat_name}.__")
...
async def unbanall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            chat = await Arankbot.get_chat(message.command[1])
            chat_id = chat.id
            chat_name = chat.title
        except Exception as e:
            return await Arankbot.error(message, f"__Invalid chatId.__\n\n`{e}`")

    ban_right = await message.chat.get_member(Arankbot.me.id)
    if not ban_right.privileges.can_restrict_members:
        return await Arankbot.delete(
            message,
    ...
    Arank = await Arankbot.edit(message, f"__Unbanning all users in {chat_name}.__")
...
async def kickall(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, f"__Kicking all users in {chat_name}.__")
...
async def deleteall(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "__Deleting all messages from this user.__")
    user = message.reply_to_message.from_user.id

    await client.delete_user_history(message.chat.id, user)
    await Arankbot.delete(Arank, "__All messages from this user has been deleted.__")
...
async def unblockall(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, f"__Unblocking all users in {chat_name}.__")
...
async def inviteAll(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give a chatId from where to invite all users.")
    ...
    if from_chat.id == -1001641358740:
        return await Arankbot.delete(message, "Can't add members from this chat!")
    ...
    await Arankbot.delete(message, f"__Added {len(targets)} users to {to_chat.title}.__")
