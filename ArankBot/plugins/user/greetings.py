...
from . import Config, HelpMenu, custom_handler, db, Arankbot, on_message
...
async def greetingsformat(_, message: Message):
    await Arankbot.edit(message, GREETINGS_FORMATTINGS)
...
async def getwelcome(client: Client, message: Message):
    ...
    if not welcome:
        return await Arankbot.edit(message, "No welcome message in this chat.")
    ...
    await Arankbot.edit(message, "Welcome message sent.")
...
async def setwelcome(client: Client, message: Message):
    ...
    await Arankbot.delete(message, f"**Welcome message saved for** {message.chat.title}")
...
async def delwelcome(client: Client, message: Message):
    if await db.is_welcome(client.me.id, message.chat.id):
        ...
        await Arankbot.delete(message, "Welcome message deleted.")
    else:
        await Arankbot.delete(message, "No welcome message in this chat.")
...
async def getgoodbye(client: Client, message: Message):
    ...
    if not goodbye:
        return await Arankbot.edit(message, "No goodbye message in this chat.")

    msg = await client.get_messages(Config.LOGGER_ID, goodbye["message"])

    await msg.copy(message.chat.id, reply_to_message_id=message.id)
    await Arankbot.edit(message, "Goodbye message sent.")
...
async def setgoodbye(client: Client, message: Message):
    ...
    await Arankbot.delete(message, f"**Goodbye message saved for** {message.chat.title}")
    await msg.reply_text(
...
async def delgoodbye(client: Client, message: Message):
    if await db.is_goodbye(client.me.id, message.chat.id):
        ...
        await Arankbot.delete(message, "Goodbye message deleted.")
    else:
        await Arankbot.delete(message, "No goodbye message in this chat.")
