...
from Arankbot.functions.templates import github_user_templates
...
async def getpfp(client: Client, message: Message):
    ...
    if message.reply_to_message:
        ...
        return await Arankbot.delete(
            Arank, f"Reply to a message or pass a username/id to get the profile pic."
        )

    if not user.photo:
        return await Arankbot.error(Arank, f"User {user.mention} has no profile pic.")
...
async def setpfp(client: Client, message: Message):
    if not message.reply_to_message:
        return await Arankbot.delete(message, "Reply to a photo to set as profile pic.")
    ...
    try:
        if message.reply_to_message.photo:
            ...
            return await Arankbot.delete(
                Arank, "Reply to a photo or video to set as profile pic."
            )
    except Exception as e:
        return await Arankbot.error(Arank, f"`{str(e)}`")

    await Arankbot.delete(Arank, "Profile pic updated successfully.")
...
async def setbio(client: Client, message: Message):
    bio = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, "Processing...")
    ...
    await Arankbot.delete(Arank, "Bio updated successfully.")
    await Arankbot.check_and_log(
        "setbio",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Bio:** `{bio}`",
    )
...
async def setname(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Pass a name to set.")

    name = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, "Processing...")
    ...
    await Arankbot.delete(Arank, "Name updated successfully.")
    await Arankbot.check_and_log(
        "setname",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Name:** `{name}`",
    )
...
async def setusername(client: Client, message: Message):
    ...
    await Arankbot.delete(Arank, "Username updated successfully.")
    await Arankbot.check_and_log(
        "setusername",
        f"**User:** {message.from_user.mention} (`{message.from_user.id}`)\n\n**Username:** `{username}`",
    )
...
async def delpfp(client: Client, message: Message):
    ...
    if not profile_pics:
        return await Arankbot.error(Arank, "No profile pics found.")
...
async def gituser(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Pass a github username to search.")
...
HelpMenu("profile").add(
    ...
    "setbio Embracing the Arankbot.",
    "To remove the bio dont pass any argument.",
).add(
    "setname", "<new name>", "Set the name of the bot.", "setname ArankBot"
).add(
    "setusername",
    "<new username>",
    "Set the username of the bot.",
    "setusername ArankBot",
