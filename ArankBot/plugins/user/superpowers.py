...
from Arankbot.functions.templates import gban_templates

from . import Config, HelpMenu, Symbols, custom_handler, db, Arankbot, on_message
...
async def globalpromote(client: Client, message: Message):
    if not message.reply_to_message:
        ...
        reason = await Arankbot.input(message) or "No reason provided."

    if user.is_self:
        return await Arankbot.delete(message, "I can't gpromote myself.")
    ...
    success = 0
    failed = 0
    Arank = await Arankbot.edit(message, f"Gpromote initiated on {user.mention}...")
    ...
    async for dialog in client.get_dialogs:
        if dialog.chat.type in [
            ...
            try:
                ...
                success += 1
                await Arank.edit(f"Gpromote initiated on {user.mention}...")
...
async def globaldemote(client: Client, message: Message):
    ...
    if user.is_self:
        return await Arankbot.delete(message, "I can't gdemote myself.")
    ...
    success = 0
    failed = 0
    Arank = await Arankbot.edit(message, f"Gdemote initiated on {user.mention}...")
    ...
    async for dialog in client.get_dialogs:
        if dialog.chat.type in [
            ...
            try:
                ...
                await dialog.chat.ban_member(user.id)
                success += 1
                await Arank.edit(f"Gdemote initiated on {user.mention}...")
            except BaseException:
                failed += 1
...
async def globalban(client: Client, message: Message):
    ...
    if user.is_self:
        return await Arankbot.delete(message, "I can't gban myself.")

    if user.id in Config.AUTH_USERS:
        return await Arankbot.delete(message, "I can't gban my auth user.")

    if user.id in Config.BANNED_USERS:
        return await Arankbot.delete(message, "This user is already gbanned.")

    if user.id in Config.DEVS:
        return await Arankbot.delete(message, "I can't gban my devs.")

    success = 0
    failed = 0
    Arank = await Arankbot.edit(message, f"Gban initiated on {user.mention}...")
    ...
    async for dialog in client.get_dialogs():
        if dialog.chat.type in [
            ...
            try:
                ...
                await dialog.chat.ban_member(user.id)
                success += 1
                await Arank.edit(f"Gban initiated on {user.mention}...")
            except BaseException:
                failed += 1
...
async def globalkick(client: Client, message: Message):
    ...
    if user.is_self:
        return await Arankbot.delete(message, "I can't gkick myself.")

    if user.id in Config.AUTH_USERS:
        return await Arankbot.delete(message, "I can't gkick my auth user.")
    ...
    if user.id in Config.DEVS:
        return await Arankbot.delete(message, "I can't gkick my devs.")

    success = 0
    failed = 0
    Arank = await Arankbot.edit(message, f"Gkick initiated on {user.mention}...")
    ...
    async for dialog in client.get_dialogs():
        if dialog.chat.type in [
            ...
            try:
                ...
                success += 1
                await Arank.edit(f"Gkick initiated on {user.mention}...")
            except BaseException:
                failed += 1
...
async def globalmute(client: Client, message: Message):
    ...
    if user.id in Config.MUTED_USERS:
        return await Arankbot.delete(message, "This user is already gmuted.")

    if user.id in Config.DEVS:
        return await Arankbot.delete(message, "I can't gmute my devs.")

    permissions = ChatPermissions(can_send_messages=False)
    success = 0
    failed = 0
    Arank = await Arankbot.edit(message, f"Gmute initiated on {user.mention}...")
    ...
    async for dialog in client.get_dialogs():
        if dialog.chat.type in [
            ...
            try:
                ...
                await dialog.chat.restrict_member(user.id, permissions)
                success += 1
                await Arank.edit(f"Gmute initiated on {user.mention}...")
            except BaseException:
                failed += 1
...
async def gbanlist(_, message: Message):
    ...
    if not gban_users:
        return await Arankbot.delete(message, "No gbanned users.")

    Arank = await Arankbot.edit(message, "Fetching gbanned users...")
...
async def gmutelist(_, message: Message):
    ...
    if not gmute_users:
        return await Arankbot.delete(message, "No gmuted users.")

    Arank = await Arankbot.edit(message, "Fetching gmuted users...")
