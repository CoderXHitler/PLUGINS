...
from . import HelpMenu, custom_handler, db, handler, Arankbot, on_message, Config
...
async def set_filter(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, f"Saving filter `{keyword}`")
    ...
    await Arankbot.delete(Arank, f"**🍀 𝖭𝖾𝗐 𝖥𝗂𝗅𝗍𝖾𝗋 𝖲𝖺𝗏𝖾𝖽:** `{keyword}`")
...
async def rmfilter(client: Client, message: Message):
    if len(message.command[0]) < 9:
        if len(message.command) < 2:
            return await Arankbot.delete(message, "Give a filter name to remove.")

        keyword = await Arankbot.input(message)
        Arank = await Arankbot.edit(message, f"Removing filter `{keyword}`")
        ...
        if await db.is_filter(client.me.id, message.chat.id, keyword.lower()):
            ...
            await Arankbot.delete(Arank, f"**🍀 𝖥𝗂𝗅𝗍𝖾𝗋 𝖱𝖾𝗆𝗈𝗏𝖾𝖽:** `{keyword}`")
        else:
            await Arankbot.delete(Arank, f"**🍀 𝖥𝗂𝗅𝗍𝖾𝗋 𝖽𝗈𝖾𝗌 𝗇𝗈𝗍 𝖾𝗑𝗂𝗌𝗍𝗌:** `{keyword}`")
    else:
        Arank = await Arankbot.edit(message, "Removing all filters...")

        await db.rm_all_filters(client.me.id, message.chat.id)
        await Arankbot.delete(Arank, "All filters have been removed.")
...
async def allfilters(client: Client, message: Message):
    if len(message.command) > 1:
        keyword = await Arankbot.input(message)
        Arank = await Arankbot.edit(message, f"Getting filter `{keyword}`")
        ...
        Arank = await Arankbot.edit(message, "Getting all filters...")
        ...
        if filters:
            ...
            await Arankbot.delete(Arank, "No filters in this chat.")
