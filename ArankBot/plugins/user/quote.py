...
async def quotely(client: Client, message: Message):
    if not message.reply_to_message:
        return await Arankbot.delete(message, "Reply to a message to quote it.")
    ...
    Arank = await Arankbot.edit(message, "__Generating quote...__")

    msg_data = []
    if cmd and cmd == "r":
        await Arank.edit("__Generating quote with reply...__")
    ...
    if not status:
        return await Arankbot.error(message, f"`{path}`")
