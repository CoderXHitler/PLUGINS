...
from Arankbot.functions.formatter import secs_to_mins
from Arankbot.functions.tools import progress

from . import HelpMenu, Symbols, Arankbot, on_message
...
async def youtube_audio(_, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Checking ...")
    status, url = YoutubeDriver.check_url(query)

    if not status:
        return await Arankbot.delete(Arank, url)

    await Arank.edit("🎼 __Downloading audio ...__")
    try:
        ...
        return await Arankbot.delete(Arank, f"**🍀 Audio not Downloaded:** `{e}`")
...
async def ytvideo(_, message: Message):
    ...
    await Arank.edit("🎼 __Downloading video ...__")
    try:
        ...
        return await Arankbot.delete(Arank, f"**🍀 Video not Downloaded:** `{e}`")
...
async def ytlink(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, "Give something to search on youtube.")
    ...
    try:
        ...
        return await Arankbot.delete(Arank, f"**🍀 Error:** `{e}`")
    ...
    await Arank.edit(text, disable_web_page_preview=True)
