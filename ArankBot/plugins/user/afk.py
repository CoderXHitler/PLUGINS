...
from Arankbot.core import Config, db, Arankbot
from Arankbot.functions.formatter import add_to_dict, get_from_dict, readable_time
...
async def afk(_, message: Message):
    if await db.is_afk(message.from_user.id):
        return await Arankbot.delete(message, "🙄 𝖨'𝗆 𝖺𝗅𝗋𝖾𝖺𝖽𝗒 𝖠𝖥𝖪!")
    ...
    await Arankbot.delete(message, "🫡 𝖦𝗈𝗂𝗇𝗀 𝖠𝖥𝖪! 𝖲𝖾𝖾 𝗒𝖺'𝗅𝗅 𝗅𝖺𝗍𝖾𝗋.")
...
async def remove_afk(_, message: Message):
    if await db.is_afk(message.from_user.id):
        ...
        Arank = await message.reply_text(
