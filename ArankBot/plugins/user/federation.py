...
from . import HelpMenu, Symbols, handler, Arankbot, on_message


@on_message("newfed", allow_stan=True)
async def newfed(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, f"Usage: {handler}newfed <fedname>")

    bot_un = "@MissRose_bot"
    await client.unblock_user(bot_un)

    fedname = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, f"__Creating new federation__ **{fedname}**")
    ...
    try:
        ...
        return await Arankbot.error(Arank, f"`{e}`")
    ...
    if "created new federation" in msg1.text.lower():
        ...
        await Arankbot.delete(Arank, f"**Failed to create federation!**\n\n`{msg1.text}`")
...
async def renamefed(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.delete(message, f"Usage: {handler}renamefed <new fedname>")

    bot_un = "@MissRose_bot"
    await client.unblock_user(bot_un)

    fedname = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, f"__Renaming federation to__ **{fedname}**")
    ...
    if "renamed your federation from" in msg1.text.lower():
        await Arank.edit(f"**𝖥𝖾𝖽𝖾𝗋𝖺𝗍𝗂𝗈𝗇 𝗋𝖾𝗇𝖺𝗆𝖾𝖽 𝗍𝗈** `{fedname}`")
    else:
        await Arankbot.delete(Arank, f"**Failed to rename federation!**\n\n`{msg1.text}`")
...
async def fedinfo(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "__Fetching federation info__")
    ...
    if "fed info" in msg1.text.lower():
        ...
        await Arankbot.delete(Arank, f"**Failed to fetch federation info!**\n\n`{msg1.text}`")
