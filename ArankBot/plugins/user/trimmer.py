...
async def videotrim(_, message: Message):
    if message.reply_to_message:
        if not (message.reply_to_message.video or message.reply_to_message.document):
            return await Arankbot.delete(message, "Reply to a video file")

    if len(message.command) < 2:
        return await Arankbot.delete(message, "Provide a valid timestamp.")
    ...
    dl_start = time.time()
    Arank = await Arankbot.edit(message, "Downloading...")
    file = await message.reply_to_message.download(
        Config.DWL_DIR,
        progress=progress,
        progress_args=(Arank, dl_start, "⬇️ Downloading..."),
    )

    dl_time = readable_time(int(time.time() - dl_start))
    await Arank.edit(f"**Downloaded in {dl_time}!** __Starting to trim video...__")
    ...
    if not os.path.lexists(file_name):
        return await Arankbot.error(Arank, f"**Error:** `{err}`")

    ul_start = time.time()
    await Arank.edit("**Trimmed!** __Uploading now...__")
    await message.reply_document(
        file_name,
        caption=caption,
        progress=progress,
        progress_args=(Arank, ul_start, "**Trimmed!** __Uploading now...__"),
    )
...
async def audiotrim(_, message: Message):
    if message.reply_to_message:
        if not (message.reply_to_message.video or message.reply_to_message.document or message.reply_to_message.audio):
            return await Arankbot.delete(message, "Reply to a video/audio file")
    ...
    dl_time = readable_time(int(time.time() - dl_start))
    await Arank.edit(f"**Downloaded in {dl_time}!** __Starting to trim audio...__")
