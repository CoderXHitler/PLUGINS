...
async def stanUsers(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "__Fetching users...__")

    users = await db.get_stans(client.me.id)
    if not users:
        return await Arankbot.delete(Arank, "__No stans found!__")
...
async def addstan(client: Client, message: Message):
    ...
    if user.id == client.me.id:
        return await Arankbot.delete(message, "__I can't be a stan of myself!__")

    if await db.is_stan(client.me.id, user.id):
        return await Arankbot.delete(message, "__This user is already a stan!__")

    await db.add_stan(client.me.id, user.id)
    await Arankbot.delete(message, f"__Added {user.mention} as a stan!__")
...
async def delstan(client: Client, message: Message):
    ...
    if await db.is_stan(client.me.id, user.id):
        ...
        await Arankbot.delete(message, f"__Removed {user.mention} from stans!__")

        Config.AUTH_USERS.remove(user.id)
        Config.STAN_USERS.remove(user.id)
    else:
        await Arankbot.delete(message, "__This user is not a stan!__")
