...
from Arankbot.functions.templates import statistics_templates, user_info_templates
...
async def userInfo(client: Client, message: Message):
    if not message.reply_to_message:
        if len(message.command) < 2:
            return await Arankbot.error(message, "Reply to a user or give username/id to get their info.")
        try:
    ...
    Arank = await Arankbot.edit(message, f"Getting info of {user.mention}...")
    ...
    if user.photo:
        ...
        await Arank.edit(user_info, disable_web_page_preview=True)
