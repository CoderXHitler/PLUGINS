...
from Arankbot.core import Symbols

from . import HelpMenu, custom_handler, db, group_n_channel, Arankbot, on_message
...
async def autopost(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Starting Autopost in this group/channel...")

    post_from = message.command[1]
    _chat = await client.get_chat(post_from)

    if not _chat:
        return await Arankbot.delete(Arank, "Invalid chat/channel id.")

    if _chat.type not in group_n_channel:
        return await Arankbot.delete(
            Arank, "You can only autopost in groups and channels."
        )

    if _chat.id == message.chat.id:
        return await Arankbot.delete(
            Arank, "You can't autopost in the same group/channel."
        )

    if _chat.id in await db.is_autopost(client.me.id, _chat.id, message.chat.id):
        return await Arankbot.delete(
            Arank, "This group/channel is already in autopost list."
        )

    await db.set_autopost(client.me.id, _chat.id, message.chat.id)

    await Arankbot.delete(
        Arank, f"Autopost started from {_chat.title} to {message.chat.title}."
    )
...
async def stop_autopost(client: Client, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Stopping Autopost in this group/channel...")
    ...
    if _chat.id not in await db.is_autopost(client.me.id, _chat.id, message.chat.id):
        return await Arankbot.delete(Arank, "This group/channel is not in autopost list.")

    await db.rm_autopost(client.me.id, _chat.id, message.chat.id)

    await Arankbot.delete(
        Arank, f"Autopost stopped from {_chat.title} to {message.chat.title}."
    )
    await Arankbot.check_and_log(
        "autopost stop",
        f"**AutoPost From:** {_chat.title} \n**AutoPost To:** {message.chat.title}\n**AutoPost By:** {client.me.mention}",
...
async def autoposts(client: Client, message: Message):
    Arank = await Arankbot.edit(message, "Getting autopost list...")

    data = await db.get_all_autoposts(client.me.id)
    if not data:
        return await Arankbot.delete(Arank, "No autoposts found.")
    ...
    await Arankbot.edit(Arank, text)
