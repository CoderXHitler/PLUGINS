...
from Arankbot.functions.scraping import is_valid_url

from . import Config, HelpMenu, Symbols, db, handler, Arankbot, on_message
...
async def google_search(client: Client, message: Message):
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give some text to search on wikipedia.")

    search_query = await Arankbot.input(message)
    Arank = await Arankbot.edit(
    ...
    try:
        ...
        return await Arank.edit(f"**DisambiguationError:**\n\n{result}")
    except exceptions.PageError:
        return await Arankbot.delete(Arank, "**Page not found.**")
...
async def googleSearch(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give some text to search on google.")

    search_query = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, f"Searching for `{search_query}` on google...")

    try:
        results = search(search_query, 7, advanced=True)
    except Exception as error:
        return await Arankbot.error(Arank, f"`{str(error)}`")
...
async def reverseSearch(_, message: Message):
    ...
    if message.reply_to_message.sticker or message.reply_to_message.photo:
        ...
        return await Arankbot.error(
            Arank, "Reply to an image/sticker to reverse search it."
        )

    await Arank.edit("Searching on google...")
    ...
    if div:
        ...
        return await Arank.edit("No results found.")
...
async def gpsLocation(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give some place name to search.")
    ...
    geolocator = Nominatim(user_agent="Arankbot")
    location: Location = geolocator.geocode(search_query)

    if not location:
        return await Arankbot.delete(Arank, "Location not found.")

    latitiude = location.latitude
    longitude = location.longitude
    address = location.address

    await Arank.reply_location(latitiude, longitude)
    await Arankbot.delete(Arank, f"**🌐 𝖯𝗅𝖺𝖼𝖾:** {address}")


@on_message("webshot", allow_stan=True)
async def webScreenshot(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give some url to take screenshot.")

    search_query = await Arankbot.input(message)
    if not is_valid_url(search_query):
        return await Arankbot.edit(message, "Invalid url.")

    Arank = await Arankbot.edit(message, f"Taking screenshot of `{search_query}`...")
    driver, resp = Driver.get()
    if not driver:
        return await Arankbot.error(Arank, resp)
    ...
    with io.BytesIO(image) as result:
        result.name = "Arankbot_Webshot.png"
        await Arank.reply_document(result)
        await Arank.delete()

    try:
        os.remove("Arankbot_Webshot.png")
    except:
        pass
...
async def cricketScore(_, message: Message):
    ...
    await Arankbot.edit(message, final)
...
async def wordMeaning(_, message: Message):
    ...
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give some word to search.")

    search_query = await Arankbot.input(message)
    Arank = await Arankbot.edit(message, f"Searching for `{search_query}`...")

    response = requests.get(BASE.format(search_query))
    if response.status_code == 404:
        return await Arankbot.delete(Arank, "Word not found.")
    ...
    if audio:
        await Arank.reply_audio(audio, caption=outStr)
        await Arank.delete()
    else:
        await Arank.edit(outStr)
...
async def translateHandler(_, message: Message):
    if message.reply_to_message:
        if len(message.command) < 2:
            return await Arankbot.edit(message, "Give some language code to translate.")
    ...
    Arank = await Arankbot.edit(message, f"Translating to `{toLang}`...")
...
async def translateCodes(_, message: Message):
    ...
    await Arankbot.edit(message, outStr)
...
async def textToSpeech(_, message: Message):
    ...
    try:
        ...
        await message.reply_audio(
            ...
            performer="ArankyAI",
            title="Arankbot TTS",
            thumb="./Arankbot/resources/images/Arankbot_logo.png",
        )
        await Arank.delete()
    except Exception as e:
        return await Arankbot.error(Arank, f"`{str(e)}`")
...
async def movieSearch(_, message: Message):
    if len(message.command) < 2:
        return await Arankbot.edit(message, "Give a movie name to search on IMDb.")
    ...
    try:
        ...
        await Arankbot.delete(Arank, "No results found.")
    except Exception as e:
...
HelpMenu("google").add(
    ...
    "google", "<query>", "Searches for the given query on google.", "google the Arankbot"
).add(
    "reverse",
    ...
    "voice I'm Aranky and this is an Text to Speech Example.",
