...
from Arankbot.functions.convert import convert_to_gif
from Arankbot.functions.tools import runcmd

from . import HelpMenu, Arankbot, on_message, Config
...
async def sticker_to_gif(_, message: Message):
    ...
    Arank = await Arankbot.edit(message, "Converting ...")
    ...
    if replied_sticker.is_animated:
        ...
        return await Arankbot.delete(Arank, "Reply to an animated/video sticker.")
    ...
    await Arankbot.delete(Arank, "Converted to gif successfully!")
...
async def sticker_to_image(_, message: Message):
    ...
    await Arankbot.delete(Arank, "Converted to image successfully!")
...
async def image_to_sticker(_, message: Message):
    ...
    await Arankbot.delete(Arank, "Converted to sticker successfully!")
...
async def file_to_image(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.document:
        return await Arankbot.delete(message, "Reply to a file to convert it to image.")

    if message.reply_to_message.document.mime_type.split("/")[0] != "image":
        return await Arankbot.delete(message, "Reply to an image file.")
...
async def image_to_file(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await Arankbot.delete(message, "Reply to an image to convert it to file.")
    ...
    await Arankbot.delete(Arank, "Converted to file successfully!")
...
async def media_to_voice(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await Arankbot.delete(message, "Reply to a media to convert it to voice.")
    ...
    if os.path.exists(voice_path):
        ...
        await Arankbot.delete(Arank, "Converted to voice successfully!")
        os.remove(voice_path)
    else:
        await Arankbot.error(Arank, f"`{err}`")
...
async def media_to_mp3(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.media:
        return await Arankbot.delete(message, "Reply to a media to convert it to mp3.")
    ...
    if os.path.exists(mp3_path):
        ...
        await Arankbot.delete(Arank, "Converted to mp3 successfully!")
        os.remove(mp3_path)
    else:
        await Arankbot.error(Arank, f"`{stderr}`")
